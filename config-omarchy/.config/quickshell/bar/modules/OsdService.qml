...
Item {
    ...
    function _cavaCommand(inputMethod) {
        return "CFG=$(mktemp \"${XDG_RUNTIME_DIR:-/tmp}/quickshell-osd-cava.XXXXXX.conf\") || exit 1; " + "trap 'rm -f \"$CFG\"' EXIT; " + "cat > \"$CFG\" <<'EOF'\n" + "[general]\n" + "bars = 22\n" + "framerate = 60\n" + "sensitivity = 100\n" + "lower_cutoff_freq = 45\n" + "higher_cutoff_freq = 10000\n" + "\n" + "[input]\n" + "method = " + inputMethod + "\n" + "source = auto\n" + "\n" + "[output]\n" + "method = raw\n" + "raw_target = /dev/stdout\n" + "data_format = ascii\n" + "ascii_max_range = 1000\n" + "bar_delimiter = 59\n" + "channels = mono\n" + "EOF\n" + "exec cava -p \"$CFG\" 2>/dev/null";
    }
    ...
    function brightnessStep(delta) {
        ...
        const sign = delta >= 0 ? "+" : "-";
        const pct = Math.abs(Math.round(delta));
        const setCommand = hasBrightnessctl && hasBrightnessDevice ? "brightnessctl set " + pct + "%" + sign + " 2>/dev/null" : "timeout 4s ddcutil setvcp 10 " + sign + " " + pct + " 2>/dev/null";
    ...
    Process {
        ...
        command: ["bash", "-lc", "printf 'wpctl=%s\\n' \"$(command -v wpctl >/dev/null 2>&1 && echo 1 || echo 0)\"; " + "printf 'brightnessctl=%s\\n' \"$(command -v brightnessctl >/dev/null 2>&1 && echo 1 || echo 0)\"; " + "printf 'brightnessdevice=%s\\n' \"$(command -v brightnessctl >/dev/null 2>&1 && brightnessctl -l 2>/dev/null | awk '/backlight/{found=1} END{exit found ? 0 : 1}' && echo 1 || echo 0)\"; " + "printf 'ddcutil=%s\\n' \"$(command -v ddcutil >/dev/null 2>&1 && echo 1 || echo 0)\"; " + "printf 'playerctl=%s\\n' \"$(command -v playerctl >/dev/null 2>&1 && echo 1 || echo 0)\"; " + "printf 'cava=%s\\n' \"$(command -v cava >/dev/null 2>&1 && echo 1 || echo 0)\""]
        running: true
        onExited: {
            root._syncCava();
        }
