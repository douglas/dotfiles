...
Item {
    ...
    function desktopExecArgs(exec) {
        const cleaned = String(exec || "").replace(/%%/g, "__QS_LITERAL_PERCENT__").replace(/%[A-Za-z]/g, "").replace(/__QS_LITERAL_PERCENT__/g, "%").trim();
        const args = [];
        let current = "";
        let quote = "";
        let escaped = false;
    ...
    Process {
        ...
        command: ["bash", "-lc", "XDG_DATA_HOME=\"${XDG_DATA_HOME:-$HOME/.local/share}\"; " + "XDG_DATA_DIRS=\"${XDG_DATA_DIRS:-/usr/local/share:/usr/share}\"; " + "for f in " + "/usr/share/applications/*.desktop " + "/usr/local/share/applications/*.desktop " + "\"$XDG_DATA_HOME\"/applications/*.desktop " + "\"$XDG_DATA_HOME\"/flatpak/exports/share/applications/*.desktop " + "/var/lib/flatpak/exports/share/applications/*.desktop " + "~/Desktop/*.desktop; do " + "  [ -f \"$f\" ] || continue; " + "  name=$(grep -m1 '^Name=' \"$f\" | cut -d= -f2-); " + "  exec=$(grep -m1 '^Exec=' \"$f\" | cut -d= -f2-); " + "  nodisplay=$(grep -m1 '^NoDisplay=' \"$f\" | cut -d= -f2-); " + "  [ \"$nodisplay\" = \"true\" ] && continue; " + "  [ -z \"$name\" ] && continue; " + "  [ -z \"$exec\" ] && continue; " + "  printf '%s|%s\\n' \"$name\" \"$exec\"; " + "done | sort -u"]
        running: true
        onExited: {
