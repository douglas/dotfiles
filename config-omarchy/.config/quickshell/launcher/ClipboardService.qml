...
Item {
    ...
    Process {
        ...
        command: ["bash", "-lc", "missing=''; " + "for c in cliphist wl-copy file; do " + "command -v \"$c\" >/dev/null 2>&1 || missing=\"$missing $c\"; " + "done; " + "missing=${missing# }; " + "if [ -z \"$missing\" ]; then echo __OK__; exit 0; fi; " + "echo \"$missing\"; exit 1"]
        running: false
        onExited: (exitCode) => {
