...
PanelWindow {
    ...
    readonly property var protectedProcessNames: ["quickshell", "hyprland", "systemd", "init", "dbus-daemon", "dbus-broker", "xwayland", "sddm", "greetd", "pipewire", "wireplumber", "networkmanager", "xdg-desktop-portal", "xdg-desktop-portal-hyprland"]
    ...
    function applyStatLine(text) {
        ...
        if (line.startsWith("MEM ")) {
            ...
            if (total > 0) {
                ...
                root.memText = (used / 1.04858e+06).toFixed(1) + " / " + (total / 1.04858e+06).toFixed(1) + " GB";
                if (root.memUsage > root.memPeak)
                    root.memPeak = root.memUsage;
        ...
        if (line.startsWith("DISK ")) {
            ...
            if (total > 0) {
                ...
                root.diskText = (used / 1.04858e+06).toFixed(0) + " / " + (total / 1.04858e+06).toFixed(0) + " GB";
            }
            return ;
        }
    ...
    function pushHistory(arr, val) {
        ...
        if (copy.length > maxHistoryPoints)
            copy.shift();
    ...
    function rebuildFilteredProcesses() {
        ...
        for (const proc of processes) {
            ...
            if (!processAdvanced && isProtectedProcess(proc))
                continue;

            if (!query || pid.includes(query) || name.includes(query))
                next.push(proc);
        ...
        next.sort((a, b) => {
            ...
            else if (processSortKey === "pid")
                cmp = (Number(a.pid) || 0) - (Number(b.pid) || 0);
            else if (processSortKey === "mem")
                cmp = (Number(a.mem) || 0) - (Number(b.mem) || 0);
            else
                cmp = (Number(a.cpu) || 0) - (Number(b.cpu) || 0);
            if (cmp === 0)
                cmp = (Number(a.pid) || 0) - (Number(b.pid) || 0);
    ...
    function setProcessSort(key) {
        ...
        if (processSortKey === key) {
            ...
            processSortKey = key;
            processSortDescending = key !== "name";
        }
    ...
    function memAbsText(memPct) {
        if (!_memTotalKb || !memPct)
            return "—";

        const kb = (_memTotalKb * Number(memPct)) / 100;
        if (kb >= 1.04858e+06)
            return (kb / 1.04858e+06).toFixed(1) + "G";
    ...
    Timer {
        ...
        interval: 2200
        repeat: false
        onTriggered: root.processNotice = ""
    }
    ...
    Timer {
        ...
        interval: 260
        repeat: false
        onTriggered: root.restartProc(psProc)
    }
    ...
    Process {
        ...
        command: ["bash", "-lc", "printf 'CPU %s\\n' \"$(head -n1 /proc/stat)\"; " + "free | awk '/^Mem:/ {printf \"MEM %d %d\\n\", $2, $7}'; " + "awk '/MemTotal/{printf \"MEMTOTAL %d\\n\", $2}' /proc/meminfo; " + "df -P / | awk 'END {gsub(/%/, \"\", $5); printf \"DISK %s %s\\n\", $2, $3}'; " + "awk 'BEGIN{found=0} NR>2 {gsub(\":\", \"\", $1); if ($1 != \"lo\") {print \"NET\", $1, $2, $10; found=1; exit}} END{if(!found) print \"NET -- 0 0\"}' /proc/net/dev"]
    ...
    Process {
        ...
        command: ["bash", "-lc", root.processAdvanced ? "ps -eo pid=,comm=,%cpu=,%mem= --sort=-%cpu" : ("ps -eo pid=,comm=,%cpu=,%mem= --sort=-%cpu | head -n " + String(root.safeProcessCap))]
        onExited: {
    ...
    Process {
        ...
        command: ["bash", "-lc", "kernel=$(uname -r 2>/dev/null); " + "osname='Omarchy'; " + "oversion=$(omarchy-version 2>/dev/null || true); " + "cpu=$(awk -F: '/model name/{print $2; exit}' /proc/cpuinfo 2>/dev/null | xargs); " + "cores=$(nproc --all 2>/dev/null || getconf _NPROCESSORS_ONLN 2>/dev/null); " + "arch=$(uname -m 2>/dev/null); " + "host=$(hostname 2>/dev/null); " + "uptime=$(awk '{d=int($1/86400);h=int(($1%86400)/3600);m=int(($1%3600)/60); printf \"%dd %dh %dm\",d,h,m}' /proc/uptime 2>/dev/null); " + "memtot=$(awk '/MemTotal/{printf \"%.1f GB\", $2/1048576}' /proc/meminfo 2>/dev/null); " + "diskdev=$(lsblk -dn -o NAME,TYPE 2>/dev/null | awk '$2==\"disk\"{print $1; exit}'); " + "cputemp=$(sensors 2>/dev/null | awk '/^(Package id 0|Tdie|CPU Temperature|Core 0)/{gsub(/[^0-9.]/,\" \",$2); print $2\"+°C\"; exit}' || cat /sys/class/thermal/thermal_zone0/temp 2>/dev/null | awk '{printf \"%.0f°C\", $1/1000}' || echo '—'); " + "gpuusage=$(nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits 2>/dev/null | awk '{print $1\"%\"}' || radeontop -d - -l 1 2>/dev/null | awk '/gpu/{match($0,/gpu ([0-9.]+)%/,a); if(a[1]) print a[1]\"%\"}' || echo '—'); " + "printf 'KERNEL\\x1f%s\\x1eOS\\x1f%s\\x1eOVERSION\\x1f%s\\x1eCPU\\x1f%s\\x1eCORES\\x1f%s\\x1eARCH\\x1f%s\\x1eHOST\\x1f%s\\x1eUPTIME\\x1f%s\\x1eMEMTOTAL\\x1f%s\\x1eDISKDEV\\x1f%s\\x1eCPUTEMP\\x1f%s\\x1eGPUUSAGE\\x1f%s\\x1e' " + "\"$kernel\" \"$osname\" \"$oversion\" \"$cpu\" \"$cores\" \"$arch\" \"$host\" \"$uptime\" \"$memtot\" \"$diskdev\" \"$cputemp\" \"$gpuusage\""]
        running: false
        onExited: {
    ...
    Rectangle {
        ...
        radius: 14
        color: root.cBg
        border.color: Qt.alpha(root.cAccent, 0.2)
        border.width: 1
        clip: true
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onPositionChanged: (mouse) => {
                        ...
                        root.cardX += mouse.x - ox;
                        root.cardY += mouse.y - oy;
                        root.clampCard();
                    }
                }
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        height: 22
                        radius: 6
                        color: root.paused ? Qt.alpha(root.cYellow, 0.2) : Qt.alpha(root.cMuted, 0.22)
                    ...
                    Rectangle {
                        ...
                        border.color: Qt.alpha(root.cBorder, 0.4)
                        border.width: 1
            ...
            Rectangle {
                Layout.fillWidth: true
                height: 1
                color: Qt.alpha(root.cBorder, 0.5)
            }
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        StatCard {
                            label: "CPU"
                            value: root.cpuUsage
                            detail: Math.round(root.cpuUsage) + "%  peak " + Math.round(root.cpuPeak) + "%"
                            accent: root.cAccent
                            peakValue: root.cpuPeak
                        }

                        StatCard {
                            label: "Memory"
                            value: root.memUsage
                            detail: root.memText + "  peak " + Math.round(root.memPeak) + "%"
                            accent: root.cGreen
                            peakValue: root.memPeak
                        }
                    ...
                    Rectangle {
                        ...
                        Column {
                            ...
                            Text {
                                text: "CPU  —  last " + root.graphWindow
                                color: root.cDim
                                font.pixelSize: Style.Typography.componentSubtitle
                                font.family: Style.Typography.text
                            }
                            ...
                            Item {
                                ...
                                Canvas {
                                    ...
                                    function drawCurvePath() {
                                        ...
                                        if (pts.length === 2) {
                                            ctx.lineTo(step, height - (pts[1] / 100 * height));
                                            return ;
                                        }
                                    ...
                                    onPaint: {
                                        ...
                                        ctx.lineTo(0, height);
                                        ctx.closePath();
                                        ctx.fillStyle = Qt.alpha(root.cAccent, 0.1);
                                        ctx.fill();
                                        // Current value label
                                        if (pts.length > 0) {
                                        ...
                                        if (peakVal > 0) {
                                            ...
                                            ctx.beginPath();
                                            ctx.arc(width - 8, peakY, 3, 0, Math.PI * 2);
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            radius: 8
                            color: root.cSurface
                            border.color: Qt.alpha(root.cBorder, 0.5)
                            border.width: 1
                            ...
                            TextInput {
                                ...
                                font.pixelSize: Style.Typography.componentSubtitle
                                font.family: Style.Typography.text
                                selectionColor: Qt.alpha(root.cAccent, 0.35)
                                selectedTextColor: root.cFg
                                onTextChanged: root.processSearch = text
                            }

                            Text {
                                anchors.left: processSearchInput.left
                                anchors.verticalCenter: parent.verticalCenter
                                text: ""
                                color: root.cDim
                                font.pixelSize: Style.Typography.componentSubtitle
                            ...
                            Rectangle {
                                ...
                                MouseArea {
                                    anchors.fill: parent
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: processSearchInput.text = ""
                                }
                        ...
                        Rectangle {
                            ...
                            height: 32
                            radius: 8
                            color: root.processAdvanced ? Qt.alpha(root.cAccent, 0.2) : Qt.alpha(root.cMuted, 0.16)
                            border.color: root.processAdvanced ? Qt.alpha(root.cAccent, 0.5) : Qt.alpha(root.cBorder, 0.5)
                            border.width: 1
                            ...
                            MouseArea {
                                anchors.fill: parent
                                cursorShape: Qt.PointingHandCursor
                                onClicked: root.processAdvanced = !root.processAdvanced
                            }
                    ...
                    Text {
                        Layout.fillWidth: true
                        text: root.processNotice !== "" ? root.processNotice : ((root.processAdvanced ? "Advanced mode" : "Safe mode") + " · Showing " + root.filteredProcesses.length + " process(es)" + (root.paused ? "  ·   paused" : ""))
                    ...
                    Item {
                        ...
                        RowLayout {
                            ...
                            PsHeaderCell {
                                text: "MEM"
                                cellWidth: root.psMemAbsW
                                rightAlign: true
                            }

                            PsHeaderCell {
                                text: "CPU bar"
                                cellWidth: root.psBarW
                            }
                    ...
                    Item {
                        ...
                        Flickable {
                            ...
                            anchors.fill: parent
                            anchors.rightMargin: root.psScrollW + 2
                            clip: true
                            contentWidth: width
                            contentHeight: psColumn.implicitHeight
                            ...
                            Column {
                                ...
                                Repeater {
                                    ...
                                    delegate: Rectangle {
                                        ...
                                        width: psColumn.width
                                        height: 30
                                        radius: 6
                                        color: isSelected ? Qt.alpha(root.cAccent, 0.12) : isKeySelected ? Qt.alpha(root.cFg, 0.08) : hovered ? Qt.alpha(root.cFg, 0.05) : "transparent"

                                        RowLayout {
                                            anchors.fill: parent
                                            anchors.leftMargin: root.psRowInset
                                            anchors.rightMargin: root.psRowInset
                                            spacing: 0
                                            ...
                                            Rectangle {
                                                ...
                                                height: 16
                                                radius: 4
                                                color: isSelected ? Qt.alpha(root.cAccent, 0.3) : Qt.alpha(root.cMuted, 0.12)
                                            ...
                                            // Kill button
                                            Rectangle {
                                                width: root.psActionW
                                                height: root.psActionW
                                                radius: 6
                                                color: protectedProc ? Qt.alpha(root.cMuted, 0.1) : Qt.alpha(root.cMuted, 0.2)
                                                border.color: Qt.alpha(root.cBorder, protectedProc ? 0.25 : 0.4)
                                                border.width: 1
                                                opacity: hovered ? 1 : 0
                                        ...
                                        MouseArea {
                                            ...
                                            onEntered: {
                                                hovered = true;
                                                root.selectedProcessIndex = index;
                                            }
                        ...
                        Rectangle {
                            anchors.top: psFlick.top
                            anchors.bottom: psFlick.bottom
                            anchors.right: parent.right
                            width: root.psScrollW - 2
                            radius: 1
                            color: Qt.alpha(root.cBorder, 0.35)
                            visible: psFlick.contentHeight > (psFlick.height + 1)

                            Rectangle {
                                width: parent.width
                                radius: 1
                                color: Qt.alpha(root.cAccent, 0.75)
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                border.color: active ? Qt.alpha(root.cAccent, 0.5) : Qt.alpha(root.cBorder, 0.45)
                                border.width: 1
                                ...
                                MouseArea {
                                    anchors.fill: parent
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: root.graphWindow = modelData
                                }
                    ...
                    GridLayout {
                        ...
                        GraphCard {
                            label: "CPU"
                            history: root.windowedHistory(root.cpuHistory)
                            lineColor: root.cAccent
                            peakValue: root.cpuPeak
                        }

                        GraphCard {
                            label: "Memory"
                            history: root.windowedHistory(root.memHistory)
                            lineColor: root.cGreen
                            peakValue: root.memPeak
                        }

                        GraphCard {
                            label: "Network download"
                            history: root.windowedHistory(root.netDownHistory)
                            lineColor: "#cba6f7"
                            peakValue: -1
                        }
                        ...
                        GraphCard {
                            ...
                            history: root.windowedHistory(root.netUpHistory)
                            lineColor: "#89dceb"
                            peakValue: -1
                        }
                ...
                GridLayout {
                    ...
                    SysInfoBlock {
                        ...
                        rows: [{
                            "k": "OS",
                            "v": root.sysOsName || "Omarchy"
                        }, {
                            "k": "Version",
                            "v": root.sysOmarchyVersion || "—"
                        }, {
                            "k": "Kernel",
                            "v": root.sysKernel || "—"
                        }, {
        ...
        Row {
            ...
            Column {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        width: 30
                        height: (parent.parent.height - 22) / 4
            ...
            Column {
                ...
                Item {
                    ...
                    Canvas {
                        ...
                        function ptY(val) {
                            return height - (val / 100 * (height - 4)) - 2;
                        }
                        ...
                        onPaint: {
                            ...
                            ctx.lineTo(0, height);
                            ctx.closePath();
                            ctx.fillStyle = Qt.alpha(lc, 0.09);
                            ctx.fill();
                            // Current value label
                            if (pts.length > 0) {
                            ...
                            if (pv > 0) {
                                ctx.beginPath();
                                ctx.arc(width - 8, ptY(pv), 3, 0, Math.PI * 2);
                                ctx.fillStyle = root.cRed;
                                ctx.fill();
                            }
                            // Crosshair + tooltip
                            if (hvX >= 0 && pts.length > 1) {
                                ...
                                ctx.strokeStyle = Qt.alpha(root.cFg, 0.25);
                                ctx.lineWidth = 1;
                                ctx.setLineDash([3, 3]);
                                ctx.stroke();
                                ctx.setLineDash([]);
    ...
    component PsHeaderCell: Item {
        ...
        Layout.preferredWidth: fill ? -1 : cellWidth
    ...
    component PsCell: Item {
        ...
        Text {
            ...
            anchors.rightMargin: 4
            text: parent.text
            color: parent.dim ? root.cDim : root.cFg
            font.pixelSize: Style.Typography.componentSubtitle
            font.family: Style.Typography.text
            horizontalAlignment: parent.rightAlign ? Text.AlignRight : Text.AlignLeft
            verticalAlignment: Text.AlignVCenter
